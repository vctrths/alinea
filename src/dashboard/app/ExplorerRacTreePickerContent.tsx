...
function useRowState(
    ...
    const containsMatches = navigationState.matchingDescendantKeys.has(
        entry.id,
    );
    ...
    return (
        <div className={styles.RacPickerTree()}>
            <Tree
                ...
                selectionMode={
                    explorer.selectionMode === "multiple"
                        ? "multiple"
                        : "single"
                }
        ...
        <TreeItem
            ...
            label={
                <div className={styles.RacPickerTree.treeItemLabel()}>
                    <span className={styles.RacPickerTree.treeItemTitle()}>
                        {label}
                    </span>
                </div>
            }
...
function ExplorerRacTreePath({
    ...
    return (
        <div className={styles.RacPickerTree.path()}>
            {rootLabel && (
                <span className={styles.RacPickerTree.path.label()}>
                    {rootLabel}
                </span>
            )}
            {parents.map((parent) => (
...
function ExplorerRacTreeLoadedPathParent({
    data,
}: {
    data: DashboardEntryData;
}) {
